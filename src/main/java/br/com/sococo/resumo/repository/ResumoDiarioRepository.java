...
public interface ResumoDiarioRepository extends JpaRepository<ResumoDiario, Long>, ResumoDiarioRepositoryQuery {
    ...
    @Transactional
    @Query("select new ResumoDiario(rd.dataLancamento, sum(rd.cocosProcessados),sum(rd.cocosDesfibrados), sum(rd.cri), sum(rd.flococo), sum(rd.oleoIndustrialTipoA), sum(rd.oleoIndustrialETE), sum(rd.torta),sum(rd.aguaDeCocoSococo), sum(rd.aguaDeCocoVerde),  sum(rd.caixaPadrao), sum(rd.porcentagemCocoGerminado), sum(rd.totalDeCacambas), sum(rd.numeroDeFardos)) from ResumoDiario rd where rd.dataLancamento = ?1 group by rd.dataLancamento order by rd.dataLancamento")
    List<ResumoDiario> findByDataLancamento(LocalDate data);

    @Transactional
    @Query("select new ResumoDiario(sum(rd.cocosProcessados),sum(rd.cocosDesfibrados), sum(rd.cri), sum(rd.flococo), sum(rd.oleoIndustrialTipoA), sum(rd.oleoIndustrialETE), sum(rd.torta),sum(rd.aguaDeCocoSococo), sum(rd.aguaDeCocoVerde),  sum(rd.caixaPadrao), sum(rd.porcentagemCocoGerminado), sum(rd.totalDeCacambas), sum(rd.numeroDeFardos)) from ResumoDiario rd where rd.mesLancamento = ?1 AND rd.anoLancamento = ?2 group by rd.mesLancamento order by rd.mesLancamento")
    ...
    @Transactional
    @Query(value = "select SUM(rd.cocos_processados), SUM(rd.cocos_desfibrados), SUM(rd.cri), SUM(rd.flococo), SUM(rd.oleo_industrial_tipo_a), SUM(rd.oleo_industrial_ete), SUM(rd.torta), SUM(rd.agua_coco_sococo), SUM(rd.agua_coco_verde), SUM(rd.caixa_padrao),  sum(rd.porcentagem_coco_germinado),  SUM(rd.total_cacambas),  SUM(rd.numero_fardos) from resumo_diario rd where weekofyear(:dataSemana) = weekofyear(rd.data_lancamento)", nativeQuery = true)
